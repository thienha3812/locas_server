...
router.post('/getplacesfromward', async (req, res, next) => {
    try {
        ...
        const places = await sequelize.query("SELECT dia_diem.*,AVG(danh_gia.rating ) as rating FROM dia_diem LEFT JOIN danh_gia ON danh_gia.ma_dd = dia_diem.ma_dd WHERE ma_xap = :ward_id GROUP BY dia_diem.ma_dd ORDER BY rating DESC", {
            replacements: {
                ward_id: ward_id
            },
            type: Sequelize.QueryTypes.SELECT
